...
(function(h) {
	...
	function X(a) {
		...
		h.each(a, function(e) {
			if (( b = e.match(/^([^A-Z]+?)([A-Z])/)) && -1 !== "a aa ai ao as b fn i m o s ".indexOf(b[1] + " "))
				c = e.replace(b[0], b[2].toLowerCase()), d[c] =
				e, "o" === b[1] && X(a[e])
		});
		a._hungarianMap = d
	}
	...
	function Da(a) {
		var b = m.defaults.oLanguage,
		    c = a.sZeroRecords;
		!a.sEmptyTable && (c && "Nenhuma ocorrência foi encontrada" === b.sEmptyTable) && E(a, a, "sZeroRecords", "sEmptyTable");
		!a.sLoadingRecords && (c && "Processando..." === b.sLoadingRecords) && E(a, a, "sZeroRecords", "sLoadingRecords");
	...
	function B(a, b, c, d) {
		...
		if (i === k)
			return a.iDrawError != e && null === j && (L(a, 0, "Requested unknown parameter " + ("function" == typeof f.mData ? "{function}" : "'" + f.mData + "'") + " for row " + b + ", column " + c, 4), a.iDrawError =
			e), j;
	...
	function Ia(a, b, c, d) {
		var e = [],
			...
			if (c === k || c === i)
				j = l[i],
				n = h.trim(a.innerHTML), j && j._bAttrSrc ? (R(j.mData._)(d, n), t(j.mData.sort, a), t(j.mData.type, a), t(j.mData.filter, a)) : q ? (j._setter || (j._setter = R(j.mData)), j._setter(d, n)) : d[i] = n;
			i++
		};
		if (f)
			for (; f; ) {
	...
	function kb(a) {
		...
		for ( c = l.length; b < c; b++)
			f = l[b],
			d = h(f.nTh).addClass(f.sClass), i && d.appendTo(e), a.oFeatures.bSort && (d.addClass(f.sSortingClass), !1 !== f.bSortable && (d.attr("tabindex", a.iTabIndex).attr("aria-controls", a.sTableId), Ma(a, f.nTh, b))), f.sTitle != d[0].innerHTML && d.html(f.sTitle), Na(a,"header")(a, d, f, n);
		i && da(a.aoHeader, g);
		...
		if (-1 !== h.inArray(!1, b))
			...
			if (0 !== i.length) {
				...
				for ( j = j ? 0 : g; j < f; j++) {
					...
					if (0 !== e) {
						var t = d[c % e];
						q._sRowStripe != t && (h(l).removeClass(q._sRowStripe).addClass(t), q._sRowStripe =
						t)
					}
				...
				c = f.sZeroRecords, 1 == a.iDraw && "ajax" == y(a) ? c = f.sLoadingRecords : f.sEmptyTable && 0 === a.fnRecordsTotal() && ( c = f.sEmptyTable), b[0] = h("<tr/>",{"class":e?d[0]:""}).append(h("<td />",{valign:"top",colSpan:aa(a),"class":a.oClasses.sRowEmpty}).html(c))[0];
	...
	function ra(a, b, c) {
		...
		a.fnServerData ? a.fnServerData.call(j, a.sAjaxSource, h.map(b, function(a, b) {
			...
			url : g || a.sAjaxSource
		})) : h.isFunction(g) ? a.jqXHR = g.call(j, b, i, a) : (a.jqXHR = h.ajax(h.extend(n, g)), g.data =
		f)
	}
	...
	function ub(a) {
		...
		for ( g = 0; g < c; g++)
			n = b[g],
			l = f[g],
			i = "function" == typeof n.mData ? "function" : n.mData, S.columns.push({
				data : i,
				name : n.sName,
	...
	function yb(a) {
		for (var b = m.ext.search,
			...
			for (var j = [],
			    ...
			    n = c.length; i < n; i++)
				e = c[i],
				d = a.aoData[e], b[f](a, d._aFilterData, e, d._aData, i) && j.push(e);
			c.length = 0;
			h.merge(c, j)
		}
	}
		...
		for ( f = a.aoData.length; d < f; d++)
			if ( h = a.aoData[d], !h._aFilterData) {
				...
				for ( g = b.length; e < g; e++)
					c = b[e], c.bSearchable ? ( i = B(a, d, e, "filter"), l[c.sType] && ( i = l[c.sType](i)), null === i && ( i = ""), "string" !== typeof i && i.toString && ( i = i.toString())) : i = "", i.indexOf && -1 !== i.indexOf("&") && (ua.innerHTML = i,
					i = Zb ? ua.textContent : ua.innerText), i.replace && ( i = i.replace(/[\r\n]/g, "")), j.push(i);
				h._aFilterData = j;
	...
	function Fa(a) {
		...
		for ( m = 0; m < i.length; m++)
			o = c[i[m]], null !== o.sWidth && (o.sWidth = Fb(o.sWidthOrig, k),
			t = !0);
		if (d || !t && !f && !e && j == aa(a) && j == n.length)
			...
			if (a.aoData.length)
				for ( m = 0; m < i.length; m++)
					t = i[m],
					o = c[t], h(Gb(a, t)).clone(!1).append(o.sContentPadding).appendTo(r);
	...
	function Hb(a, b) {
		for (var c,
			...
			c = B(a, f, b, "display") + "",
			c = c.replace($b, ""),
			c = c.replace(/&nbsp;/g, " "), c.length > d && ( d = c.length,
			e =
			f);
		return e
	}
	...
	function V(a) {
		...
		for ( a = 0; a < n.length; a++) {
			...
			for ( c = f.length; b < c; b++)
				g = f[b],
				j = e[g].sType || "string", n[a]._idx === k && (n[a]._idx = h.inArray(n[a][1], e[g].asSorting)), d.push({
					src : i,
					col : g,
					dir : n[a][1],
					index : n[a]._idx,
	...
	function Va(a, b, c, d) {
		...
		c && a.oFeatures.bSortMulti ? ( c = h.inArray(b, G(e, "0")), -1 !== c ? ( b = g(e[c], !0), null === b && 1 === e.length && ( b = 0), null === b ? e.splice(c, 1) : (e[c][1] = f[b], e[c]._idx =
		b)) : (e.push([b, f[0], 0]), e[e.length - 1]._idx = 0)) : e.length && e[0][0] == b ? ( b = g(e[0]), e.length = 1, e[0][1] = f[b], e[0]._idx =
	...
	function L(a, b, c, d) {
		...
		if (b)
			...
			b = b.sErrMode || b.errMode, a && u(a, null, "error", [a, d, c]), "alert" == b)
			alert(c);
		else {
			if ("throw" == b)
				throw Error(c);
	...
	var m = function(a) {
		...
		this.each(function() {
			...
			if ("table" != this.nodeName.toLowerCase())
				...
				g = h.isArray(e.iDeferLoading), o._iRecordsDisplay = g ? e.iDeferLoading[0] : e.iDeferLoading, o._iRecordsTotal = g ? e.iDeferLoading[1] : e.iDeferLoading);
				var r = o.oLanguage;
			...
			for ( h = l.length; g < h; g++) {
				...
				if ("table" === b)
					...
					for ( n = t.length; i < n; i++)
						f = t[i],
						f = "cell" === b ? c.call(o, l[g], f.row, f.column, g, i) : c.call(o, l[g], f, g, i, m), f !== k && e.push(f)
				}
			}
			return e.length || d ? ( a = new r(l, a ? e.concat.apply([], e) : e),
			b = a.selector, b.rows = p.rows, b.cols = p.cols, b.opts = p.opts, a) : this
		},
	...
	s("rows().remove()", "row().remove()", function() {
		...
		this.iterator("row", function(b, c, d) {
			...
			for ( h = e.length; g < h; g++)
				if ( i = e[g],
				l = i.anCells, null !== i.nTr && (i.nTr._DT_RowIndex = g), null !== l) {
					i = 0;
			...
			if (a !== k && g.bVisible !== a) {
				if (a) {
					...
					for ( n = j.length; i < n; i++)
						l = j[i].nTr,
						f = j[i].anCells, l && l.insertBefore(f[c], f[m] || null)
				} else
	...
	m.defaults = {
		...
		aaSortingFixed : [],
		ajax : null,
		aLengthMenu : [3, 5, 10, 25, 50, 100],/* Números de linhas por página */
		aoColumns : null,
		aoColumnDefs : null,
		aoSearchCols : [],
		...
		oLanguage : {
			...
			sEmptyTable : "Nenhuma ocorrêcia foi encontrada",
